...
| Settings | `KetPlusCM::settings_ui`, `src/app/SettingsDialog.h` | `SettingsPage` supports stable id/title, `hasChanges`, `resetToDefaults`, `apply`, and `changed`. `SettingsHost::registerPage` validates stable metadata, ordering, collisions and ownership; selection is addressable by id. `SettingsDialog::addPage` remains as a compatibility adapter. | Existing editor-only constructor and `settingsSaved` signal remain for compatibility. Save still applies every registered page and Reset still resets the whole dialog. | Add an explicit fallible/atomic apply contract only when a real consumer requires it; extension preview rollback remains consumer-owned. |
